...
- **Фоновый режим**: Игра продолжает работать даже при неактивном окне
- **Автостоп**: Автоматическая остановка после случайного количества транзакций (100-300)
- **Адаптивный интерфейс**: HUD автоматически подстраивается под размер экрана
- **Простое управление**: Переключение с клавиатуры ('S' — старт/стоп, 'A' — автостоп)
...
1. **Запустите игру** на странице стресс-теста MegaETH (Пробел, чтобы динозавр начал двигаться)
...
|---------|----------|
| **S** | Старт/Стоп бота |
| **A** | Включить/выключить автостоп |

### 🎯 Автостоп

Функция автоматической остановки бота после случайного количества транзакций:

1. **Нажмите 'A'** — включится автостоп, сгенерируется случайная цель (100-300 TX)
2. **Нажмите 'S'** — бот стартует и счётчик сбросится
3. При достижении цели — бот **автоматически остановится**
4. При следующем старте — новая случайная цель

В HUD отображается прогресс: `🎯 50/247`
...
- **v1.3 (v66)**: Фоновый режим и автостоп
  - 🔄 Игра продолжает работать при неактивном окне
  - 🎯 Автостоп после случайного количества TX (100-300)
  - 📱 Адаптивный интерфейс для узких экранов
  - ⌨️ Новая горячая клавиша 'A' для автостопа
