...
1. **Компенсация триггера**: Увеличивает расстояние срабатывания пропорционально времени кадра
   - При 60 FPS (16.67ms): стандартное расстояние
   - При 30 FPS (33ms): расстояние × 2
   - При 20 FPS (50ms): расстояние × 3

2. **Аварийный прыжок**: Дополнительная проверка на критически близкое расстояние
   - Срабатывает если препятствие пропущено из-за лага
   - Формула: `speed × 2 + 30` пикселей

3. **Защита от двойных прыжков**: Минимальный интервал 200ms между прыжками
